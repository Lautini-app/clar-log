...
function WordReportSection({ period }: { period: ObservationPeriod }) {
  ...
  const handleExportPdf = async (report: WordReport) => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(14);
    doc.text("clar.log ÃÂ¢ÃÂÃÂ Wortbericht", 14, 18);
    doc.setFontSize(10);
  ...
  return (
    <SectionCard title="Wortbericht" subtitle="Anonymisierte Zusammenfassung ÃÂ¢ÃÂÃÂ max. 2ÃÂÃÂ pro Monat.">
...
const NEGATIVE_EMOTIONS = new Set(["Verzweifelt", "Traurig", "Melancholisch", "ÃÂÃÂngstlich", "WÃÂÃÂ¼tend", "Stumpf/Taub"]);
...
function moodScore(log: DayLog): number | undefined {
  ...
  const scored = entries.map(([emotion, rawVal]) => { const v = Number(rawVal); return NEGATIVE_EMOTIONS.has(emotion) ? 5 - v : v; }).filter((v): v is number => !isNaN(v) && isFinite(v));
  ...
  return (
    <div className="space-y-4 pb-32">
      <header className="pt-2">
        ...
        {period && (
          <p className="mt-1 text-sm text-muted-foreground">
            {period.medications.map((m) => `${m.name} ${m.mg}mg`).join(" ÃÂÃÂ· ")}
            {period.medications.length > 0 ? " ÃÂÃÂ· " : ""}
          ...
          {[
            ...
            <div key={label} style={{ background: "#f1efe8", borderRadius: 12, padding: "10px 12px" }}>
              ...
              <p style={{ fontSize: 20, fontWeight: 500, color }}>
                {value != null ? value.toFixed(1) : "ÃÂ¢ÃÂÃÂ"}
              </p>
      ...
      {days.length > 1 && (
        <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 16, padding: 16 }}>
          ...
          <div style={{ display: "flex", gap: 12, fontSize: 10, color: "#888780", marginTop: 4 }}>
            <span style={{ color: "#1D9E75" }}>Ã¢ÂÂ Stimmung</span>
            <span style={{ color: "#BA7517" }}>Ã¢ÂÂ Energie</span>
            <span style={{ color: "#534AB7" }}>Ã¢ÂÂ Fokus</span>
            <span style={{ color: "#E8850A" }}>Ã¢ÂÂ Appetit</span>
            <span style={{ color: "#7A6C5D" }}>Ã¢ÂÂ KÃÂ¶rper</span>
          </div>
        </div>
      )}
      ...
      {expanded && (() => {
        ...
        return (
          <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 16, padding: 16 }}>
            <h3 style={{ fontSize: 13, fontWeight: 500, marginBottom: 12 }}>{d.label} ÃÂ¢ÃÂÃÂ Detail</h3>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {[
                ...
                <div key={label} style={{ padding: "8px 10px", background: "#f1efe8", borderRadius: 10 }}>
                  ...
                  <p style={{ fontSize: 18, fontWeight: 500, color }}>{value != null ? value.toFixed(1) : "ÃÂ¢ÃÂÃÂ"}</p>
            ...
            {d.hasRebound && (
              <div style={{ marginTop: 8, padding: "8px 10px", background: "#FCEBEB", borderRadius: 10 }}>
                <p style={{ fontSize: 11, color: "#A32D2D", fontWeight: 500 }}>
                  ...
                  {d.rebound != null ? ` ÃÂÃÂ· StÃÂÃÂ¤rke ${d.rebound}/4` : ""}
                </p>
              </div>
            )}
            <p style={{ fontSize: 11, color: "#888780", marginTop: 8 }}>{d.slots}/3 Slots ausgefÃÂÃÂ¼llt</p>
          </div>
        );
      })()}
                  ...
                  <div key={entry.id} className="rounded-2xl border border-border bg-background p-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="rounded-xl bg-primary/10 p-2 text-center">
                        ...
                        <p className="font-semibold text-primary">{day?.mood != null ? day.mood.toFixed(1) : "ÃÂ¢ÃÂÃÂ"}</p>
                      </div>
                      <div className="rounded-xl bg-primary/10 p-2 text-center">
                        <p className="text-muted-foreground">FremdeinschÃÂÃÂ¤tzung</p>
                        <p className="font-semibold text-primary">{entry.mood ?? "ÃÂ¢ÃÂÃÂ"}</p>
                      </div>
                    </div>
                    {entry.note && <p className="mt-2 text-xs text-muted-foreground">ÃÂ¢ÃÂÃÂ{entry.note}ÃÂ¢ÃÂÃÂ</p>}
                  </div>
                );
              })}
          </div>
      ...
      {/* Detailkurven je Item Ã¢ÂÂ Arzt-PDF */}
      {days.length > 1 && (
        <div style={{ background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 16, padding: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {([
              ...
              { key: "body" as const, label: "KÃÂ¶rper", color: "#7A6C5D" },
              ...
              return (
                <div key={key} style={{ background: "#f9f8f4", borderRadius: 10, padding: "8px 10px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 4 }}>
                    <p style={{ fontSize: 10, color: "#888780" }}>{label}</p>
                    <p style={{ fontSize: 14, fontWeight: 500, color }}>{avgVal != null ? avgVal.toFixed(1) : "Ã¢ÂÂ"}</p>
                  </div>
